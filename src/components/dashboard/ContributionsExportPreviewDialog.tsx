...
import ContributionsPrintLayout, {
  ContributionMemberRow,
  ContributionPeriodColumn,
} from "./ContributionsPrintLayout";

interface ContributionsExportPreviewDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  periods: ContributionPeriodColumn[];
  members: ContributionMemberRow[];
  totals: Record<string, number>;
}

export default function ContributionsExportPreviewDialog({
  open,
  onOpenChange,
  periods,
  members,
  totals,
}: ContributionsExportPreviewDialogProps) {
  ...
  const handleDownload = () => {
    ...
    a.href = url;
    a.download = `CARAS-Monthly-Contributions-${format(
      new Date(),
      "yyyy-MM-dd"
    )}.pdf`;
    a.click();
    ...
    <>
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-5xl w-full h-[85vh] flex flex-col">
          <DialogHeader>
            <DialogTitle>Contributions Report Preview</DialogTitle>
            <DialogDescription>
          ...
          <div className="flex-1 min-h-0 border rounded-md bg-muted/30 overflow-hidden">
            {generating || !pdfUrl ? (
              ...
              <iframe
                src={pdfUrl}
                title="Contributions PDF Preview"
