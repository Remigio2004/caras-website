...
interface ExpensesPrintLayoutProps {
...
// DonationsPrintLayout / PenaltiesPrintLayout.
const ExpensesPrintLayout = forwardRef<HTMLDivElement, ExpensesPrintLayoutProps>(
  ({ rows, total }, ref) => {
    const itemColWidth = 220;
    const dateColWidth = 130;
    const amountColWidth = 120;
    const noteColWidth = 300;
    const tableWidth = itemColWidth + dateColWidth + amountColWidth + noteColWidth;
        ...
        >
          <thead>
            <tr style={{ backgroundColor: "#fffecb" }}>
              <th style={{ width: `${itemColWidth}px`, padding: "6px 8px" }}>
                <span style={{ position: "relative", top: "-6px" }}>
                  <div style={cellContentStyle("left", true)}>Item:</div>
                </span>
              </th>
              <th style={{ width: `${dateColWidth}px`, padding: "6px 8px" }}>
                <span style={{ position: "relative", top: "-6px" }}>
                  <div style={cellContentStyle("center", true)}>Date Spent:</div>
                </span>
              </th>
            ...
            {rows.map((r) => (
              <tr key={r.id}>
                <td style={{ padding: "4px 8px", border: "none" }}>
                  <div style={cellContentStyle("left")}>{r.item_description}</div>
                </td>
                <td style={{ padding: "4px 8px", border: "none" }}>
                  <div style={cellContentStyle("center")}>
                    {formatDate(r.date_spent)}
                  </div>
                </td>
                <td style={{ padding: "4px 8px", border: "none" }}>
                  <div style={cellContentStyle("center")}>
            ...
            <tr style={{ borderTop: "2px solid #000" }}>
              <td colSpan={4} style={{ padding: "6px 8px", border: "none" }}>
                <div style={cellContentStyle("left", true)}>
                  Total Expenses: {formatPeso(total)}
                </div>
              </td>
            </tr>
          </tbody>
...
ExpensesPrintLayout.displayName = "ExpensesPrintLayout";
...
export default ExpensesPrintLayout;