...
import DonationsPrintLayout, { DonationExportRow } from "./DonationsPrintLayout";

interface DonationsExportPreviewDialogProps {
  open: boolean;
...
export default function DonationsExportPreviewDialog({
  open,
  onOpenChange,
  rows,
  total,
}: DonationsExportPreviewDialogProps) {
  ...
  const handleDownload = () => {
    ...
    a.href = url;
    a.download = `CARAS-Donations-Report-${format(new Date(), "yyyy-MM-dd")}.pdf`;
    a.click();
    ...
    <>
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-5xl w-full h-[85vh] flex flex-col">
          <DialogHeader>
            <DialogTitle>Donations Report Preview</DialogTitle>
            <DialogDescription>
      ...
      {open &&
        createPortal(
          <div style={{ position: "fixed", left: "-9999px", top: 0 }}>
            <DonationsPrintLayout ref={printRef} rows={rows} total={total} />
          </div>,
          document.body
        )}
    </>
