...
import PenaltiesPrintLayout, { PenaltyExportRow } from "./PenaltiesPrintLayout";

interface PenaltiesExportPreviewDialogProps {
  open: boolean;
...
export default function PenaltiesExportPreviewDialog({
  open,
  onOpenChange,
  rows,
  totals,
}: PenaltiesExportPreviewDialogProps) {
  ...
  const handleDownload = () => {
    ...
    a.href = url;
    a.download = `CARAS-Penalty-Report-${format(new Date(), "yyyy-MM-dd")}.pdf`;
    a.click();
    ...
    <>
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-5xl w-full h-[85vh] flex flex-col">
          <DialogHeader>
            <DialogTitle>Penalty Report Preview</DialogTitle>
            <DialogDescription>
      ...
      {open &&
        createPortal(
          <div style={{ position: "fixed", left: "-9999px", top: 0 }}>
            <PenaltiesPrintLayout ref={printRef} rows={rows} totals={totals} />
          </div>,
          document.body
        )}
    </>
